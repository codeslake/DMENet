...
def resNet(t_image, reuse=False, scope = "resNet"):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        n = Conv2d(n, 1000, (h, w), act=tf.nn.relu, padding='VALID', W_init=w_init, b_init=b_init, name='fcn1')
...
def resNet_test(t_image, reuse=False, scope = "resNet_test"):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        '''
        #h, w = n.outputs.get_shape().as_list()[1:3]
        n = Conv2d(n, 1000, (71, 71), act=tf.nn.relu, padding='SAME', W_init=w_init, b_init=b_init, name='fcn1')
        n = Conv2d(n, 512, (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, b_init=b_init, name='fcn2')
        output = Conv2d(n, 1, (1, 1), act=tf.nn.tanh, padding='SAME', W_init=w_init, b_init=b_init, name='fcn3')
        n = ReshapeLayer(output, [-1, 1], name='flatten')
